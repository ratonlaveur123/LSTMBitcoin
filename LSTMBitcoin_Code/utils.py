...
def compute_white_reality_check(df, price_col):

    new_df = df.copy()

    new_df['detrended_price_col'] = detrendPrice.detrendPrice(new_df[price_col]).values
    new_df['detrended_pct_change'] = new_df['detrended_price_col'].pct_change()
    new_df['detrended_system_returns'] = new_df['detrended_pct_change'] * new_df['signal']
    WhiteRealityCheckFor1.bootstrap(new_df['detrended_system_returns'])
...
def compute_metrics(df):
    ...
    new_df['system_equity']=np.cumprod(1+new_df.system_returns) -1
    system_cagr=(1+new_df.system_equity.tail(n=1))**(period/new_df.shape[0])-1
    new_df.system_returns= np.log(new_df.system_returns+1)
    system_sharpe = np.sqrt(period)*np.mean(new_df.system_returns)/np.std(new_df.system_returns)

    new_df['mkt_equity']=np.cumprod(1+new_df.mkt_returns) -1
    mkt_cagr=(1+new_df.mkt_equity.tail(n=1))**(period/new_df.shape[0])-1
    new_df.mkt_returns= np.log(new_df.mkt_returns+1)
    mkt_sharpe = np.sqrt(period)*np.mean(new_df.mkt_returns)/np.std(new_df.mkt_returns)
